            ...
            <form class="uk-form uk-form-stacked" @submit.prevent="update">
                ...
                <div class="uk-grid uk-grid-width-1-2 uk-form-row">
                    ...
                    <div>
                        ...
                        <input class="uk-width-1-1" id="form-height" type="text" :disabled="!isVimeo && !isYoutube" :placeholder="$trans('auto')" v-model="video.data.height">
                    </div>
                </div>
...
<script>
    export default {
        ...
        watch: {
            videoSource: _.debounce(function() {
                if(this.video.data.src != this.videoSource) {
                    this.$set(this.video.data, 'src', this.videoSource);
                }
            }, 500),
        },
