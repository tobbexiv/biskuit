...
<validation-observer id="post" v-slot="{ handleSubmit }" slim>
  <form @submit.prevent="handleSubmit(save)" v-cloak>
    <div class="uk-margin uk-flex uk-flex-between uk-flex-wrap" uk-margin>
      ...
      <div uk-margin>
        <a class="uk-button uk-button-default uk-margin-small-right" :href="$url.route('admin/blog/post')">{{ post.id ? 'Close' : 'Cancel' | trans }}</a>
